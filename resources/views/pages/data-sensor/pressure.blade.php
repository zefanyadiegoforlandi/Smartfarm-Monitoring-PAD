...
<div class="container mx-auto px-4 md:px-8 lg:px-16 xl:px-20">
    <div class="mt-5 flex flex-col md:flex-row md:items-center justify-between">
        <div class="flex items-center justify-start">
            ...
            <p class="font-semibold text-3xl md:text-4xl text-[#416D14]">Tekanan</p>
        </div>
    ...
    <div class="flex items-center justify-end group mt-5">
        <a href="{{ route('history.Pressure') }}" class="font-medium text-[16px] md:text-[20px] text-[#416D14] me-1 group-hover:brightness-125 group-active:brightness-150">lihat selengkapnya</a>
    ...
    <div class="overflow-x-auto mt-2">         
        <table id="pressure-table" class="w-full">
            <thead class="bg-[#ECF0E8]">
                <tr>
                    ...
                    <th class="p-2 text-[#416D14] uppercase">Tekanan</th>
                </tr>
            </thead>
            ...
            <tbody id="data-container">
                @foreach ($dataTabel as $ds)
                    <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-[#ecf0e82e]' : 'bg-white' }}">
                        ...
                        <td class="py-2 px-4 border-b text-center">{{ $ds['Pressure'] }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
...
<script>
    ...
    function reloadData() {
        $.get('/update-data-table-Pressure', function(response) {
...
// Memisahkan data label dan tekanan dari data sensor
var labels = tableData.map(entry => entry.TimeAdded);
var pressure = tableData.map(entry => entry.Pressure);

// Menentukan batas atas dan bawah untuk sumbu y
var maxDataValue = Math.max(...pressure);
var minDataValue = Math.min(...pressure);
...
function fetchDataAndUpdateChart() {
    fetch('/update-data-grafik-Pressure')
        .then(response => response.json())
        .then(data => {
            var newData = data.Pressure;
