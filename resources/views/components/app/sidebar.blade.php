<div>
  ...
  <div style="display: flex; flex-direction: column; align-items: center; height: 100vh; mt-10">
    <div id="animatedRectangle" style="width: 206px; height: 86px; background-color: #ffffff; display: none; margin-bottom: 10px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.2); border-radius: 5px;">
        <div class="mt-3">
            @foreach ([
                ['route' => 'read-auth.edit', 'icon' => 'images/user_button_icon.svg', 'label' => session('user_name'), 'logout' => false],
                ['route' => 'logout', 'icon' => 'images/logout_icon.svg', 'label' => 'Log Out', 'logout' => true]
            ] as $item)
                <div class="flex items-center {{ !$loop->first ? 'mx-4 mt-4' : 'ml-4' }} {{ Request::is($item['route']) ? 'text-[#416D14]' : '' }}">
                    <img src="{{ asset($item['icon']) }}" alt="{{ $item['label'] }} Icon" class="w-5 h-5 object-cover mx-2">
                    @if($item['logout'])
                        <form method="POST" action="{{ route($item['route']) }}">
                            @csrf
                            <a href="{{ route($item['route']) }}" class="mx-2 text-lg {{ Request::is($item['route']) ? 'text-[#416D14]' : 'text-gray-500' }} hover:text-[#416D14]" onclick="event.preventDefault(); this.closest('form').submit(); this.classList.add('text-[#416D14]');">{{ $item['label'] }}</a>
                        </form>
                    @else
                        <form action="{{ route($item['route']) }}">
                            @csrf
                            <button class="flex items-center justify-start {{ Request::is($item['route']) ? 'text-[#416D14]' : 'text-gray-500' }} text-lg hover:text-[#416D14]" style="background-color: transparent; border: none;" onclick="this.classList.add('text-[#416D14]'); setTimeout(function() { window.location.href='{{ route($item['route']) }}'; }, 10);">
