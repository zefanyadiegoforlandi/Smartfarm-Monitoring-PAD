...
class DataTemperatureController extends Controller
{
    public function getData_Temperature()
    {
        try {
            ...
            if ($response->successful()) {
                ...
                return view('/pages/data-sensor/temperature', compact('paginator', 'dataSensor', 'dataTabel', 'sensors'));
            } else {
    ...
    public function updateDataGrafik_Temperature()
    {
        try {
            $id_sensor = session('id_sensor');
            ...
            if ($response->successful()) {
                ...
                $Temperature = $sortedData->pluck('Temperature')->toArray();
                $TimeAdded = $sortedData->pluck('TimeAdded')->toArray();

                return response()->json(['Temperature' => $Temperature, 'TimeAdded' => $TimeAdded]);
            } else {
                return response()->json(['error' => 'Failed to retrieve data sensors'], $response->status());
            }
        } catch (\Exception $e) {
    ...
    public function updateDataTable_Temperature()
    {
        try {
            $id_sensor = session('id_sensor');
    ...
    public function history_Temperature()
    {
        try {
            $id_sensor = session('id_sensor');
            $id_lahan = session('id_lahan');
            ...
            if ($response->successful()) {
                ...
                return view('pages.history.temperature', compact('paginator', 'sensors'));
            } else {
                return response()->json(['error' => 'Failed to retrieve data sensors'], $response->status());
            }
        } catch (\Exception $e) {
