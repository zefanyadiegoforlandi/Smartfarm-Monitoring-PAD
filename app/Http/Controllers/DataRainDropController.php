...
class DataRainDropController extends Controller
{
    public function getData_RainDrop()
    {
        try {
            ...
            if ($response->successful()) {
                ...
                return view('/pages/data-sensor/raindrop', compact('paginator', 'dataSensor', 'dataTabel', 'sensors'));
            } else {
    ...
    public function history_Raindrop()
    {
        try {
            $id_sensor = session('id_sensor');
            $id_lahan = session('id_lahan');
            ...
            if ($response->successful()) {
                ...
                return view('pages.history.raindrop', compact('paginator', 'sensors'));
            } else {
                return response()->json(['error' => 'Failed to retrieve data sensors'], $response->status());
            }
        } catch (\Exception $e) {
