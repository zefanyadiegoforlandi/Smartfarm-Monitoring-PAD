...
class DataPersentaseKelembapanTanahController extends Controller
{
    public function getData_PersentaseKelembapanTanah()
    {
        try {
            ...
            if ($response->successful()) {
                ...
                $dataSensor = $sortedData->map(function ($item) {
                    return [
                        'PersentaseKelembapanTanah' => $item['PersentaseKelembapanTanah'],
                ...
                return view('/pages/data-sensor/persentasekelembapantanah', compact('paginator', 'dataSensor', 'dataTabel', 'sensors'));
            } else {
    ...
    public function updateDataGrafik_PersentaseKelembapanTanah()
    {
        try {
            $id_sensor = session('id_sensor');
            ...
            if ($response->successful()) {
                ...
                $PersentaseKelembapanTanah = $sortedData->pluck('PersentaseKelembapanTanah')->toArray();
                $TimeAdded = $sortedData->pluck('TimeAdded')->toArray();

                return response()->json(['PersentaseKelembapanTanah' => $PersentaseKelembapanTanah, 'TimeAdded' => $TimeAdded]);
            } else {
                return response()->json(['error' => 'Failed to retrieve data sensors'], $response->status());
            }
        } catch (\Exception $e) {
    ...
    public function updateDataTable_PersentaseKelembapanTanah()
    {
        try {
            $id_sensor = session('id_sensor');
    ...
    public function history_PersentaseKelembapanTanah()
    {
        try {
            $id_sensor = session('id_sensor');
            $id_lahan = session('id_lahan');
            ...
            if ($response->successful()) {
                ...
                return view('pages.history.persentasekelembapantanah', compact('paginator', 'sensors'));
            } else {
                return response()->json(['error' => 'Failed to retrieve data sensors'], $response->status());
            }
        } catch (\Exception $e) {
