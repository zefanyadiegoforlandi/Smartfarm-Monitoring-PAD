    ...
    public function getData_AirQuality()
{
    try {
        ...
        if ($response->successful()) {
            ...
            // Urutkan data berdasarkan TimeAdded dalam urutan menurun (terbaru di atas)
            $sortedData = collect($dataSensors)->sortByDesc('TimeAdded')->values();

            // Ambil 20 data terbaru untuk chart dan 10 data terbaru untuk tabel
            $dataSensor = $sortedData->slice(0, 20)->map(function ($item) {
                return [
            ...
            $dataTabel = $sortedData->slice(0, 10)->map(function ($item) {
                return [
                    'AirQuality' => $item['AirQuality'],
                    'TimeAdded' => $item['TimeAdded'],
                    'id_sensor' => $item['id_sensor']
            ...
            $lengthAwarePaginator = new LengthAwarePaginator(
                collect($dataSensor)->forPage($currentPage, $perPage), 
                count($dataSensor), 
                $perPage, 
                $currentPage 
            );
